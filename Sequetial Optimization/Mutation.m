...
for ii = 1:(Population_number-1)*(VariableNumber-1)
    ...
    Fitness_Muted=Fitness(2:Population_number,2:4)';
for ii=1:count-1
    Variable_Selection = rem(Muted_Choromosome(ii),3);
...
Muted_Fit=Fitness(2:Population_number,2:4);
for ii = 1:Population_number-1
    if Fitness_Muted_2(ii,3) >= Fitness_Muted_2(ii,2) 
       Muted_Fit(ii,:) = Fitness_Muted_2(ii,:);
    else
       Muted_Fit(ii,1) = Fitness_Muted_2(ii,1);
    end
end
Fitness(2:Population_number,2:4) = Muted_Fit; 
